export default function AddonInitializationFile({ fileName, fileSize, onRemove, isLoading }) {
  ...
  return (
    <section className="p-10 rounded-lg border-4 border-indigo-900 bg-indigo-950 mb-5 mt-5 flex items-center" data-testid="addon-initialization-file">
      ...
      <button
        onClick={onRemove}
        type="button"
        className="rounded-lg text-sm px-2 py-1.5 outline outline-transparent focus:outline-offset-2 focus:outline-red-600 bg-gray-800 text-gray-400 hover:text-white hover:bg-red-700 transition-colors duration-300 disabled:outline-0 disabled:bg-gray-800 disabled:text-gray-400"
        disabled={isLoading}
